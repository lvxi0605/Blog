		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			inputAddRule.on('click',function() {
				...
				// this will not trigger onchange listener
				return false;
			});
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			cm = p.columns[j];
			// create it here so it can be referentiated in the onchange listener
			...
			"drag" : function (opts) {
				return $.extend({
					...
					stop :function(ev,ui) {
						...
						// if we have a empty row inserted from start listener try to delete it
						for (i=0;i<$.data($t,"dnd").connectWith.length;i++){
				...
				return $.extend({
					...
					drop: function(ev, ui) {
						...
						if($.data(tid[0],"dnd").beforedrop && $.isFunction($.data(tid[0],"dnd").beforedrop) ) {
							//parameters to this callback - listener, element, data to be inserted, sender, reciever
